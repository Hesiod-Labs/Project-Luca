...
/**
 * User associated with the {@link Account}. Each user has a {@link #username}, which is based on the user's first,
...
public class User
{
    ...
    //TODO private double pctHoldings;
    ...
    public User(String first, String middleInit, String last, String password, boolean permission, double contribution)
    {
        ...
        this.userBalance = new Balance(contribution); //TODO Should contributions be a part of the userBalance?
        userBalance.updateBalance(userBalance); //TODO Or should it be based on current pctHoldings?
    }
        ...
        if(this.getPermissionStatus())
        {
            ...
            if(!Account.getAccountUsers().contains(newUser))
            {
                ...
                if(!Account.getAccountName().isEmpty())
                    System.out.println("User successfully added to " + Account.getAccountName() + ".");
                else
                    System.out.println("User successfully added to the account.");
            }
            // Otherwise, the user already exists in the account and does not need to be added.
            else
            {
                if(!Account.getAccountName().isEmpty())
                    System.out.println("User already exists in " + Account.getAccountName() + ".");
                else
    ...
    public void removeUser(User userToRemove)
    {
        ...
        if(this.getPermissionStatus())
        {
            ...
            if(Account.getAccountUsers().contains(userToRemove))
            {
                ...
                System.out.println("User successfully removed from " + Account.getAccountName() + ".");
                System.out.println("User successfully removed from the account.");
            }
            else
            {
                // Otherwise, the user does not exist in the account, so cannot be removed.
                if(!Account.getAccountName().isEmpty())
                    System.out.println("User does not exist in " + Account.getAccountName() + ".");
                else
    ...
    /**
     * @return Dollar amount the user has added to the funds for investing.
     */
    public double getUserContribution()
    {
        return userContribution.getCurrentValue();
    }
